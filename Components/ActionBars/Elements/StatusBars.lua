...
local GetTimeToWellRested = GetTimeToWellRested or function() return nil end
	...
	if (factionData) then
		...
		if (ns.IsRetail) then
			if (factionID and C_Reputation.IsFactionParagon(factionID)) then
				local paragonInfo = C_Reputation.GetFactionParagonInfo(factionID)
				local currentValue, threshold, hasRewardPending = paragonInfo.currentValue, paragonInfo.threshold, paragonInfo.hasRewardPending
		...
		for i = 1, C_Reputation.GetNumFactions() do
			...
			if factionInfo then
				...
				if (factionName == name) then
					...
					standingDescription = factionInfo.description
