	...
	if (not self.bar) then
		...
		toggle.UpdateAlpha = function(self)
			if (self.mouseOver) or (IsShiftKeyDown() and IsControlKeyDown()) or (self.bar:IsShown()) then
				...
				-- Делаем кнопку слегка видимой даже когда не наведена
				self:SetAlpha(0.3)
			end
		end
		...
		-- Show toggle on Shift+Ctrl
		RegisterStateDriver(toggle, "state-vis", "[modifier:ctrl,modifier:shift]show;hide")
...
MicroMenu.OnInitialize = function(self)
	-- Проверяем настройку включения микроменю
