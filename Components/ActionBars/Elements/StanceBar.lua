...
local style = function(button)
	...
	-- WoW 12.0 optimization: Set alpha once instead of hooking SetCooldown
...
StanceBar.UpdatePosition = function(self)
	...
	local db = ns.db.global.stancebar
	self.Bar:ClearAllPoints()
	self.Bar:SetPoint("BOTTOMRIGHT", UIParent, "BOTTOM", db.positionX or 380, (db.positionY or 84 + ActionBars:GetBarOffset()) / self.Bar.scale)
end
...
StanceBar.OnInitialize = function(self)
	if (not ns.db.char.actionbars.showStanceBar) then
		self:Disable()
		return
	end
	self:SpawnBar()
